...
impl ToolExecutor for GenerateProjectReportTool {
    ...
    async fn execute(&self, arguments: Option<Value>) -> Result<CallToolResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if include_issues {
            match self.api_client.list_issues(Some(args.project_id), Some(1000), None, None, None, None, None, None, None, None, None, None, None).await {
                Ok(issues_response) => {
...
impl ToolExecutor for GetDashboardDataTool {
    ...
    async fn execute(&self, arguments: Option<Value>) -> Result<CallToolResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // 2. Přehled úkolů
        match self.api_client.list_issues(None, Some(1000), None, None, None, None, None, None, None, None, None, None, None).await {
            Ok(issues_response) => {
